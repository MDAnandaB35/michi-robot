# --- ASYNC CHANGE: Import necessary async libraries ---
import asyncio
import aiofiles
...
import logging
import tempfile
from contextlib import asynccontextmanager # Changed to async context manager
...
# --- ASYNC CHANGE: Use AsyncOpenAI client ---
from openai import AsyncOpenAI, OpenAIError 
import paho.mqtt.client as mqtt
from pydub import AudioSegment
from quart_cors import cors # Use quart_cors
from gtts import gTTS  # Add this import for Google TTS fallback
...
class MQTTClient:
    # MQTT connection is kept simple. For high-throughput MQTT, an async-native
    # library like `aio-mqtt` would be a further improvement.
    ...
    async def apublish_command(self, intent: str): # ASYNC Method
        ...
        with Timer("MQTT publish"):
            try:
                ...
                # --- ASYNC CHANGE: Run blocking publish call in a separate thread ---
...
def detect_wake_word_fuzzy(text: str, threshold: int = 85) -> bool: # This is CPU-bound, no async needed
...
async def concurrent_response_generation(message: str, core: Main) -> Tuple[str, str]:
    with Timer("Concurrent response generation"):
        # --- ASYNC CHANGE: Schedule intent classification and doc retrieval to run at the same time ---
        ...
        # --- ASYNC CHANGE: Wait for both concurrent tasks to complete ---
    ...
    with Timer("LLM response generation"):
        # --- ASYNC CHANGE: Use ainvoke for the final, non-blocking LLM call ---
...
async def agenerate_speech_elevenlabs(text: str, save_path: str) -> None: # ASYNC Method
    with Timer("TTS generation"):
        logger.debug("Generating TTS for text: %s", text)
        try:
            # --- ASYNC CHANGE: Run blocking ElevenLabs call in a separate thread ---
            ...
            # --- ASYNC CHANGE: Write to file asynchronously ---
            async with aiofiles.open(save_path, "wb") as f:
                # The result from generate might be a generator or bytes
            ...
            try:
                ...
                # gTTS is blocking and only supports writing to file synchronously
...
async def detect_wakeword():
    with Timer("Full wakeword detection"):
        ...
        async with temp_audio_file("wakeword_") as wav_path:
            ...
            # Optionally, implement is_valid_speech as an async function or comment out for now
            # if not await is_valid_speech(wav_path):
...
async def process_input():
    with Timer("Full input processing"):
        ...
        async with temp_audio_file("upload_") as wav_path:
            ...
            try:
                # --- FIX: Read the file's bytes asynchronously first ---
                async with aiofiles.open(wav_path, "rb") as f:
                    audio_data = await f.read()

                with Timer("Audio transcription"):
                    # --- FIX: Pass the raw bytes (in a tuple) to the OpenAI client ---
                    transcript = await openai_client.audio.transcriptions.create(
                        model="whisper-1",
                        file=("audio.mp3", audio_data), # Pass a tuple of (filename, content)
                        language="id"
                    )
