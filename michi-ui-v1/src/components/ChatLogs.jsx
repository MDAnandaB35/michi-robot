...
// --- Mock API & Data ---
const mockChatLogs = [
    { id: 1, input: "Hello there!", response: "Hi! How can I help you today?", time: "2025-06-27T10:30:00Z" },
    { id: 2, input: "What's the weather like?", response: "It's sunny in your area.", time: "2025-06-27T10:31:00Z" },
    { id: 3, input: "Thanks!", response: "You're welcome!", time: "2025-06-27T10:32:00Z" },
    { id: 4, input: "Any updates on my order?", response: "Let me check that for you.", time: "2025-06-28T14:00:00Z" },
    { id: 5, input: "Okay, waiting.", response: "Your order has been shipped and will arrive tomorrow.", time: "2025-06-28T14:01:00Z" },
    { id: 6, input: "Great news!", response: "Glad I could help!", time: "2025-06-28T14:02:00Z" },
    { id: 7, input: "Book a flight to New York", response: "For what dates?", time: "2025-07-01T09:00:00Z" },
    { id: 8, input: "Next Tuesday", response: "Searching for flights...", time: "2025-07-01T09:01:00Z" },
];
...
const ChatMessage = ({ text, time, isInput }) => (
    <div className={`flex ${isInput ? 'justify-end' : 'justify-start'} mb-4`}>
        <div className={`max-w-xs md:max-w-md lg:max-w-lg px-4 py-3 rounded-2xl shadow-md ${isInput ? 'bg-blue-500 text-white rounded-br-none' : 'bg-gray-100 text-gray-800 rounded-bl-none'}`}>
            <p className="text-sm">{text}</p>
            <p className={`text-xs mt-1 ${isInput ? 'text-blue-200' : 'text-gray-500'} text-right`}>
                {formatTime(time)}
            </p>
        </div>
    </div>
...
const IOSDatePicker = ({ isOpen, onClose, onSelect, availableDates }) => {
    ...
    const years = useMemo(() => Array.from(dateMap.keys()).sort((a, b) => a - b), [dateMap]);
    ...
    const days = useMemo(() => {
        if (!dateMap.has(selectedYear) || !dateMap.get(selectedYear).has(selectedMonth)) return [];
        return Array.from(dateMap.get(selectedYear).get(selectedMonth)).sort((a, b) => a - b);
    ...
    return (
        <div className="fixed inset-0 bg-opacity-30 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
            <div className="bg-white w-full max-w-sm rounded-2xl shadow-lg p-4" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center pb-3 border-b">
                    ...
                    <button onClick={handleSelect} className="text-blue-500 font-bold">Done</button>
                </div>
                <div className="flex justify-center items-center py-4 relative">
                    <PickerColumn values={months} selectedValue={selectedMonth} onSelect={setSelectedMonth} unit="month" />
                    <PickerColumn values={days} selectedValue={selectedDay} onSelect={setSelectedDay} unit="day" />
                    <PickerColumn values={years} selectedValue={selectedYear} onSelect={setSelectedYear} unit="year" />
                </div>
            </div>
        </div>
    );
...
export default function App() {
    ...
    const availableDates = useMemo(() => {
        ...
        return Array.from(dates).map(dateStr => new Date(dateStr).toISOString()).sort((a, b) => new Date(b) - new Date(a));
    }, [logs]);
        ...
        <div className="font-sans flex justify-center h-full">
            <div className="w-full mx-4 h-full flex flex-col">
                <div className="p-3 border-b flex flex-wrap gap-2 items-center justify-between bg-gray-200 rounded-2xl">
                    ...
                    <h2 className="text-xl font-bold text-gray-800 mb-2 text-center">Michi Chatlogs {error ? <p className="text-sm text-red-500">{error}</p> : <p className="text-sm text-gray-600">Browse conversation history.</p>}</h2>
                    <button
                        onClick={() => setIsPickerOpen(true)}
                        disabled={availableDates.length === 0}
                        className="flex items-center gap-2 px-4 py-2 text-sm rounded-full transition-all bg-gray-100 text-gray-700 hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                       {filterDate === 'all' ? 'Select Date' : new Date(filterDate).toLocaleDateString([], { month: 'short', day: 'numeric' })}
                    </button>
                </div>
                ...
                <div className="flex-1 p-4 overflow-y-auto">
                    {isLoading ? (
                       <Loader />
                        ...
                        Object.keys(groupedLogs).sort((a,b) => new Date(a) - new Date(b)).map(date => (
                            <React.Fragment key={date}>
                                <DateSeparator date={date} />
                                {groupedLogs[date].sort((a, b) => new Date(a.time) - new Date(b.time)).map((msg) => (
                                     msg.type === 'input' 
                                     ? <ChatMessage key={msg.msgId} text={msg.input} time={msg.time} isInput={true} />
                                     : <ChatMessage key={msg.msgId} text={msg.response} time={msg.time} isInput={false} />
                                ))}
                            </React.Fragment>
                        ))
                    ) : (
